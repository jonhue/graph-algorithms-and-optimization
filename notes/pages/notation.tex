...
\vspace{0cm}\begin{longtable}{p{2.5cm}l}
...
\begin{longtable}{p{2.5cm}l}
   ...
   $\psi(\sS)$ & sparsity of a cut $\sS$, $\frac{c(\sS)}{\min\{|\sS|, |\sV \setminus \sS|\}}$ \\
   $\psi(G)$ & sparsity of a graph $G$, $\min_{\emptyset \subset \sS \subset \sV} \psi(S)$ \\
   \addlinespace
...
\vspace{0.5cm}\section*{\smallcaps{Optimization}}\vspace{-0.5cm}
\begin{longtable}{p{2.5cm}l}
   $L(\vy, \vx, \vs)$ & Lagrangian of an optimization problem with primal variables $\vy$ and dual variables $\vx, \vs$ \\
\end{longtable}
\end{fullwidth}
