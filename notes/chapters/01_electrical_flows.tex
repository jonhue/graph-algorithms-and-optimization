...
\end{align} We define $\mR \defeq \diag\{\vr(e)\}_{e \in \sE}$ and then have that Ohm's law\index{Ohm's law} can be expressed as, \begin{align}
    \trans{\mB}\ex = \mR\ef, \quad\text{or equivalently,}\quad \mR^{-1}\trans{\mB}\ex = \ef.
\end{align} If the net flow constraint is satisfied, this yields, \begin{align}
    \underbrace{\mB\mR^{-1}\trans{\mB}}_{\text{Laplacian}}\ex = \mB\ef = \vd. \label{eq:ohms_law_and_net_flow_constraint}
\end{align}
...
We have seen that finding electrical voltages $\ex$ or the electrical flow $\ef$ is equivalent, we can go from one to the other and back. So let us first focus on how we can find electrical voltages $\ex$.

In \cref{eq:ohms_law_and_net_flow_constraint}, we saw that electrical voltages satisfy $\mL\ex = \vd$, obeying by Ohm's law and satisfying the net flow constraint. A standard approach to reframe the solution to such a system of linear equations as the result of an optimization problem, is to consider the cost function, \begin{align}
...
\end{align} We can therefore write the \emph{electrical energy}\index{electrical energy} dissipated by routing a current $\ef$ (or equivalently with electrical voltages $\ex$) as, \begin{align}
    \e(\ef) \defeq \sum_{e \in \sE} \vr(e) \ef(e)^2 = \trans{\ef}\mR\ef = \trans{\ex}\mL\ex \defeq \mathcal{E}(\ex). \margintag{using Ohm's law, $\ef = \mR^{-1}\mB\ex$}
\end{align}

\begin{rmk}
Given electrical voltages $\ex \perp \vOne$, we can also write the electrical energy as \begin{align}
    \e(\ex) = \trans{\ex}\mL\ex = \trans{\vd}\ex = \trans{\vd}\pinv{\mL}\vd \eqdef \e(\vd), \margintag{using $\mL\vx = \vd$ and $\pinv{\mL}\mL\vx = \vx$ as $\vx \perp \ker{\mL}$} \label{eq:electrical_energy_demands}
...
\end{align} It turns out that $\s{\vf}$ is precisely the electrical flow $\ef$, that is, $\s{\vf}$ satisfies Ohm's law.\footnote{Proof in \cref{lem:a2}.} This indicates that the two above optimization problems are intimately related: both yield the electrical flow (or equivalently, electrical voltages). In fact, it can be shown that $\e(\ef) = -c(\ex)$, where we think about maximizing $-c(\vx)$ instead of minimizing $c(\vx)$.\footnote{Proof in \cref{lem:a3}.}. More generally, $\e(\vf) \geq -c(\vx)$ for any flow $\vf$ routing $\vd$ and voltages $\vx$.\footnote{Proof in \cref{lem:a4}.} So, for any voltages $\vx$, the value of $-c(\vx)$ is a lower bound on the minimum electrical energy $\e(\ef)$.
